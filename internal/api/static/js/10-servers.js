...
function renderServerList() {
  ...
  const el = $id('splist');
  if (!list.length) {
    el.innerHTML = '<div class="sp-noservers">Серверов нет.<br>Добавьте vless/trojan/ss/hy2/tuic ссылку в настройках.</div>';
...
async function importSrvUrlFromPanel() {
  ...
  if (!isSupportedServerURI(url)) { showToast('Поддерживаются vless, trojan, ss, hysteria2, tuic', 'warn'); return; }
  try {
    const r = await fetch(API + '/servers', {
      method:'POST',
